...
// TODO: use a stored refresh token
pub async fn authenticate(auth: &OidcAuthConfig) -> Result<OidcUserInfo, AuthError> {
    ...
    let redirect_server = RedirectServer::new(auth.redirect_port).await?;
    ...
    .await
    .map_err(|e| AuthError::Discovery(Box::new(e)))?;
    ...
    .set_redirect_uri(
        RedirectUrl::new(format!("http://localhost:{}", redirect_server.port())).expect("??? wtf"),
    );
    ...
    // TODO: TODO VERY IMPORTANT. We can't just block here, that's insane. We have to propagate information to the user.
    // But blocking for now is fine I guess.
    ...
    let token_response = client
        ...
        .await
        .map_err(|e| AuthError::CodeExchange(Box::new(e)))?;

    // Extract the ID token claims after verifying its authenticity and nonce.
    let id_token = token_response
        .id_token()
        .ok_or_else(|| AuthError::NoIdToken)?;
